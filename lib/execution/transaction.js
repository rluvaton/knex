      ...
      init
        ...
        .then((transactor) => {
          ...
          if (result && result.then && typeof result.then === 'function') {
            result
              .then(transactor.commit.bind(transactor))
              .catch(transactor.rollback.bind(transactor));
          }
          return null;
        })

        // TODO - check whether the rejecter can be changed 
...
function makeTransactor(trx, connection, trxClient) {
  ...
  transactor.savepoint = transactor.transaction.bind(transactor);

  if (trx.client.transacting) {
    transactor.commit = trx.release.bind(trx, connection);
    transactor.rollback = trx.rollbackTo.bind(trx, connection);
  } else {
    transactor.commit = trx.commit.bind(trx, connection);
    transactor.rollback = trx.rollback.bind(trx, connection);
  }

  transactor.isCompleted = trx.isCompleted.bind(trx);
